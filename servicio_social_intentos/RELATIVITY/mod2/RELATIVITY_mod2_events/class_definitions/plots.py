...
#
# from servicio_social.variables import *
#
...
#         Optionally uses a logarithmic scale for the x-axis.
#
...
#         if isinstance(component, int):  # this "if" statement will allow us to either input one integer (the component we're looking for) or a list of integers (the components we'd like to see)
#             component = [component]
#
#         fig_plot, axis_plot = plt.subplots()
#
#         if x_log_scale:
#             axis_plot.set_xscale('log')  # Set "log_scale=True" when calling this method to enable the logarithmic x-axis
#         if y_log_scale:
#             axis_plot.set_yscale('log')  # Set "log_scale=True" when calling this method to enable the logarithmic x-axis
#
#         # Plot each requested component
#         for comp_idx in component:
#             # Ensure comp_idx is within the valid range
...
#             axis_plot.scatter(self.x, plot_idx, marker='.', label=f'"{r_list[comp_idx]}(x)" \n {len(self.x)} points')
#
#         axis_plot.set(xlabel='distance', ylabel='component of "r(x)"')
