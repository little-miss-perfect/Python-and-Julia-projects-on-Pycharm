...
solver = ShootingSolver(
    ...
    t_span=(0, 1e20)  # this shows up to where we truncate our "infinite" domain
)
...
if np.sign(f_a) == np.sign(f_b):
    print("⚠️ Initial bracket invalid. Dynamically expanding search range...")
    # Expand bracket exponentially until signs differ
    expansion_factor = 2.0
    max_expansions = 5
    for _ in range(max_expansions):
        initial_bracket = (
            initial_bracket[0] / expansion_factor,
            initial_bracket[1] * expansion_factor
        )
        f_a = solver._residual(initial_bracket[0])
        f_b = solver._residual(initial_bracket[1])
        print(f"Trying bracket: {initial_bracket}, residuals: [{f_a:.3e}, {f_b:.3e}]")
        if np.sign(f_a) != np.sign(f_b):
            break
    else:
        raise ValueError("Could not find valid bracket after expansions.")
