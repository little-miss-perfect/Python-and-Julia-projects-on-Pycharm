...
class DerivativeFunctions:
    def __init__(self, f, f1, f2, f3, f32, rho, xrho, T):
        ...
        self.xrho = xrho
        self.T    = T
        self.scaling_factor = 1e10  # Reduce variable magnitudes to avoid overflow
    ...
    scaling_factor = 1e10  # Reduce variable magnitudes to avoid overflow
    ...
    def _scalar_n1(self, x, n, m, R, R1, P, Ms, Mb):
        if x == 0:
            ...
            return (
                (n / (x * (2 * self.f1(R) + x * R1 * self.f2(R)) + 1e-12))
            ) * (
                    ((x ** 2) * m * (self.f(R) - R * self.f1(R) + 2 * 8 * np.pi * P))
                    + 2 * (m - 1) * self.f1(R)
                    - 4 * x * R1 * self.f2(R)
            ) / self.scaling_factor  # ✅ Rescaled to prevent overflow
    ...
    #
    #         # Fix division by zero in m1_val computation
    #         m1_val = (
    #                          m / (x * (2 * self.f1(R) + x * R1 * self.f2(R)) + epsilon)  # ✅ Added epsilon
    #                  ) * (
    #                          2 * self.f1(R) * (1 - m)
    #                          + 16 * np.pi * m * (x ** 2) * self.rho
    #                          + ((m * (x ** 2)) / 3) * (R * self.f1(R) + self.f(R) + 16 * np.pi * self.T)
    #                          + x * R1 * (self.f2(R) / (self.f1(R) + epsilon)) * (  # ✅ Added epsilon
    #                                  ((m * (x ** 2)) / 3) * (2 * R * self.f1(R) - self.f(R) + 8 * np.pi * self.T)
    #                                  - 8 * np.pi * m * (x ** 2) * (-1 * self.rho + P)
    #                                  + 2 * (1 - m) * self.f1(R)
    #                                  + 2 * x * R1 * self.f2(R)
    ...
    def _scalar_R2(self, x, n, m, R, R1, P, Ms, Mb):
        if x == 0:
            return ((2 * self.f(R) - self.f1(R) * R + 8 * np.pi * self.T) / (
                        9 * self.f2(R) + epsilon))  # ✅ Added epsilon
            ...
            n1_val = (
                             n / (x * (2 * self.f1(R) + x * R1 * self.f2(R)) + epsilon)  # ✅ Added epsilon
                     ) * (
                             (x ** 2) * m * (self.f(R) - R * self.f1(R) + 2 * 8 * np.pi * P)
                             + 2 * (m - 1) * self.f1(R)
                             - 4 * x * R1 * self.f2(R)
            ...
            return (
                    (m * (8 * np.pi * self.T + 2 * self.f(R) - self.f1(R) * R)) / (
                        3 * self.f2(R) + epsilon)  # ✅ Added epsilon
                    - self.f32(R) * (R1 ** 2)
                    + m1_val * (R1 / (2 * m + epsilon))  # ✅ Added epsilon
                    - ((n1_val / (2 * n + epsilon)) + (2 / (x + epsilon))) * R1  # ✅ Added epsilon
            )
    ...
    def _scalar_DP(self, x, n, m, R, R1, P, Ms, Mb):
        ...
        if x == 0:
            ...
            return - (self.rho + P) * (n1_val / (2 * n + epsilon))  # ✅ Added epsilon
        else:
            return 0
